...
public class VicUserSessionServiceImpl implements VicUserSessionService {
    ...
    public VicUserSessionServiceImpl(PropFetcher propFetcher) {
        if (propFetcher == null) {
            throw new IllegalArgumentException("constructor argument cannot be null");
        }
        this._propFetcher = propFetcher;
    }

    @Override
    public boolean isCurrentUserVsphereAdmin() {
        return this._propFetcher.isSessionUserVsphereAdmin();
    }
}
