...
import {
  ContainerNetwork,
  VchContainerNetworkView,
  Vch, VchAuth, VchCompute, VchContainer, VchEndpoint,
  VchNetwork, VchRegistry,
  VchStorage, VchView, serverCertSourceAutogenerated, serverCertSourceExisting, TlsCaView,
} from '../../../interfaces/vch';
import {getThumbprintFromCertificate, parseCertificatePEMFileContent} from '../certificates';

/**
 * Converts the received payload from the api (a Vch model) into a view model (VchView model)
 */
export function vchViewToVch(payload: VchView): Vch {
  ...
  if (payload.networks) {
    ...
    if (payload.networks.containerNetworks && payload.networks.containerNetworks.length) {
      vchNetwork.container = payload.networks.containerNetworks.map((containerUi: VchContainerNetworkView) => {
...
/**
 * Converts the payload generated on ui side (a VchView model) into an api model (Vch model)
 */
export function vchToVchView (vch: Vch): VchView {
  const uiModel: VchView = {
    ...
    registry: {
      insecureRegistry: vch.registry.insecure,
      whitelistRegistry: vch.registry.whitelist,
      useWhitelistRegistry: (vch.registry.whitelist && vch.registry.whitelist.length > 0),
      registryCa: vch.registry.certificate_authorities.map((cert, index) => {
        return ({
          name: `Registry-cert-${index + 1}`,
          expires: parseCertificatePEMFileContent(cert.pem).expires,
          content: cert.pem,
          thumbprint: getThumbprintFromCertificate(cert.pem)
        })
...
function vchNetworkContainerApiToUi(apiContainers: ContainerNetwork[]): VchContainerNetworkView[] {
  return apiContainers.map( container => {
    const containerUi: VchContainerNetworkView = {
