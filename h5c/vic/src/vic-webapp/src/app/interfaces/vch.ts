...
export interface VchEndpoint {
  ...
  operations_credentials?: VchOperations;
}

// VCH Operations
export interface VchOperations {
    password: string;
    user: string;
    grant_permissions?: true
}

export interface VchOperationsView {
  opsUser: string;
  opsPassword: string;
  opsGrantPerms?: boolean;
}
...
export type VchViewTypes = VchView | VchGeneralView | VchComputeView | VchStorageView | VchNetworkView | VchSecurityView |
  VchRegistryView | VchOperationsView;
export type VchViewKeys = 'general' | 'computeCapacity' | 'storageCapacity' | 'networks' | 'security' | 'registry' | 'operations';
